...
if (filter_has_var(INPUT_POST, 'Apply')) {
	...
	if ($View->Controller($Output, 'GenerateSSLKeyPair', filter_input(INPUT_POST, 'SetSerial'))) {
		PrintHelpWindow(_NOTICE('SSL key pair generated. You should restart httpd or the system now.'), 'auto', 'INFO');
	}
	else {
...
This may be necessary if these files are corrupted and you cannot see any graphs displayed, such as when the system clock is set to a very distant time.'));
	...
	<tr class="evenline">
		<td class="title">
			<?php echo _TITLE('Generate SSL key pair').':' ?>
		</td>
		<td>
			<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
				<?php echo _TITLE('set serial to') ?>
				<input type="text" name="SetSerial" style="width: 50px;" maxlength="4" value="1" />
				<input type="submit" name="Generate" value="<?php echo _CONTROL('Generate') ?>" onclick="return confirm('<?php echo _NOTICE('Are you sure you want to generate SSL key pair?') ?>')"/>
			</form>
		</td>
		<td class="none">
			<?php
			PrintHelpBox(_HELPBOX('This button allows you to generate or regenerate the SSL key pair for httpd. You should restart httpd or the system to use the new SSL key pair. Make sure you increment the serial number on each new release.'));
			?>
		</td>
	</tr>
	<tr class="oddline">
