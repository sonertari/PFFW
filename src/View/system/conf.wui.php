...
if (count($_POST)) {
	...
	if (filter_has_var(INPUT_POST, 'User') && filter_has_var(INPUT_POST, 'NewPassword') && filter_has_var(INPUT_POST, 'NewPasswordAgain')) {
		...
		if (in_array($User, $ALL_USERS)) {
			...
			PrintHelpWindow(_NOTICE('FAILED').': '._NOTICE('pffw currently supports only admin username'), 'auto', 'ERROR');
...
<code>/usr/bin/chpass -a "admin:$(/usr/bin/encrypt `/bin/echo -n soner123 | sha1 -`):1000:1000::0:0:PFFW admin:/var/empty:/var/www/htdocs/pffw/Controller/sh.php"</code>'));
				?>
			</td>
		</tr>
		<tr class="oddline">
	...
	<tr class="evenline">
		...
		<td class="none">
			<?php
			PrintHelpBox(_HELPBOX('This is the log level for the pffw web interface. Logs at this level and up will be recorded in WUI and Controller log files. This setting does not effect other services.'));
			?>
		</td>
	</tr>
	<tr class="oddline">
