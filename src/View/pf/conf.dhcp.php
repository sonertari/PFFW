...
function PrintDHCPInterfaceForm()
{
	...
	<tr class="<?php echo $class ?>">
		...
		<td class="none">
			<?php PrintHelpBox(_HELPBOX('This is the list of interfaces DHCP server listens and distributes IP leases. Here you should add the interface(s) to start the DHCP server for.')) ?>
		</td>
	</tr>
	<?php
}
...
function PrintDHCPOptionForm($option)
{
	...
	$helpmsgs= array(
		'domain-name-servers'	=>	_HELPBOX('This is the DNS server internal clients use. The default is the internal IP address of the system.'),
		'routers'				=>	_HELPBOX('This is the gateway internal clients use to reach the external network. The default is the internal IP address of the system.'),
		);

	$titles= array(
		'domain-name-servers'	=>	_TITLE('Name server'),
		'routers'				=>	_TITLE('Gateway'),
		'subnet-mask'			=>	_TITLE('Subnet mask'),
		'broadcast-address'		=>	_TITLE('Broadcast address'),
		);
...
function PrintDHCPRangeOptionForm()
{
	...
	<tr class="<?php echo $class ?>">
		...
		<td class="none">
			<?php PrintHelpBox(_HELPBOX('You should take into account that there may be computers with static IP addresses or using BOOTP protocol, and choose a range accordingly.')) ?>
		</td>
	</tr>
	<?php
}
...
if (filter_has_var(INPUT_POST, 'Delete')) {
	/// @attention filter_input() does not support array vars.
	/// @todo Check what filter*() function supports POST vars of array type.
	...
	$View->Controller($Output, 'SetOptions', filter_input(INPUT_POST, 'domain-name-servers'), filter_input(INPUT_POST, 'routers'), filter_input(INPUT_POST, 'subnet-mask'),
		filter_input(INPUT_POST, 'broadcast-address'), filter_input(INPUT_POST, 'lower_range'), filter_input(INPUT_POST, 'upper_range'));
}
