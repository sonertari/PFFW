...
<table id="nvp">
	...
	if ($View->Controller($Output, 'GetListenOn')) {
		...
		<tr class="oddline">
			...
			<td class="none">
				<?php PrintHelpBox(_HELPBOX('The IP address that the DNS server listens on.')) ?>
			</td>
		</tr>
		<?php
	}
	if ($View->Controller($Output, 'GetForwarders')) {
		...
		<tr class="evenline">
			...
			<td class="none">
				<?php PrintHelpBox(_HELPBOX('Forwarder is the IP address of a DNS server that the system itself queries. In simple setups, this is usually the IP address of a DSL modem which functions as gateway to the Internet.')) ?>
			</td>
		</tr>
		<?php
	}
